...
fitness=@(x) objfuninverse(x,W,R,alpha,power,datasize,bandwidth,a,b,c,rtt);
[x]=ga(fitness,4,A,B,Aeq,Beq,lb,ub,@(x)constraintsinverse(x,R,W,bandwidth),intcon);
workload=zeros(1,4);latency=zeros(1,4);E=zeros(1,4);precision=zeros(1,4);

for i=1:4
    workload(i)=round(W*bandwidth(i)/sum(bandwidth));
end
...
filename = ['rawresult/inverse/inverse-raw-' num2str(W) '-' num2str(R) '-' num2str(alpha)];
